...
#    VIIRS_L2_SST_OBPG_QUICKLOOK
#    VIIRS_L2_SST_OBPG_REFINED
#
...
$g_routine_name = "generic_level2_uncompressor";
...
#------------------------------------------------------------------------------------------------------------------------
sub generic_level2_uncompressor_driver {
    # Set the input retrieved from command line.

    my $i_datatype          = shift;  # Always "sea_surface_temperature".
    my $i_datasource        = shift;  # The data sources are: {VIIRS,MODIS_A,MODIS_T}
    my $i_processing_type   = shift;  # Processing types are: {VIIRS,MODIS_A,MODIS_T}
    ...
    my $debug_module = "generic_level2_uncompressor_driver:";
    my $debug_mode   = 1;
    ...
    while (($index_to_sst_sst4_list < $num_sst_sst4_files) && ($num_uncompressed_files_created < $NUM_FILES_TO_PROCESS)) {
        ...
        if ($debug_mode) {
           print $debug_module . "sst_filename_compressed_file $sst_filename_compressed_file\n";
           print $debug_module . "i_processing_type            $i_processing_type\n";
           print $debug_module . "decompress_to_directory      $decompress_to_directory\n";
           print $debug_module . "i_value_move_instead_of_copy $i_value_move_instead_of_copy\n";
           print $debug_module . "status                       $status\n";
        ...
        if ($debug_mode) {
            print $debug_module . "GHRSST_MODIS_L2_STREAM_IS_HISTORICAL [" . $ENV{GHRSST_MODIS_L2_STREAM_IS_HISTORICAL} . "]\n";
            if ($ENV{GHRSST_MODIS_L2_STREAM_IS_HISTORICAL} ne "true") {
                print  $debug_module . "use_perl_script_to_get_refined_flag $use_perl_script_to_get_refined_flag\n";
            }
            print  $debug_module . "quicklook_or_refined_flag $quicklook_or_refined_flag\n";
        }
...
#        # Only move the file if the user requested it, otherwise perform a copy only.
...
#        } else {
#            # The user did not request a move but only a copy.
#            $call_system_command_str = "cp $i_sst_filename $out_filename";
#            $move_or_copy_command = "FILE_COPY";
#            system("cp $i_sst_filename $out_filename");
#            log_this("INFO",$g_routine_name,"$move_or_copy_command $i_sst_filename $out_filename");
#        }
...
# End of of main subroutine generic_level2_uncompressor_driver.
...
sub perform_decompression_task {
    ...
    if ($g_use_file_locking_mechanism_flag) {
        ...
        if ($ok_to_decompress_file_flag == 0) {
            $o_decompress_status = 1;
            return ($o_decompress_status,$o_uncompressed_filename);
        }
    }
    ...
    if ($file_is_compressed_flag == 1)  {
        my ($o_file_action_status,$o_full_output_filename);
        my $i_file_action;
        if ($i_value_move_instead_of_copy eq 'yes') {
            $i_file_action = "FILE_MOVE";
            ($o_file_action_status,$o_full_output_filename) = apply_file_management_action($i_file_action,$i_compressed_filename,$upper_level_output_directory);
        } else {
            $i_file_action = "FILE_COPY";
            ($o_file_action_status,$o_full_output_filename) = apply_file_management_action($i_file_action,$i_compressed_filename,$upper_level_output_directory);
        }
        ...
        my $temporary_compressed_filename = $o_full_output_filename;
        ...
        if (index($name_only,".bz2") >= 0) {
            ...
            log_this("ERROR",$g_routine_name,"This program only support .bz2 or .gz uncompression.  Cannot uncompress file $temporary_compressed_filename");
            exit(1);
        }
        ...
        if ($? == -1) {
            ...
            log_this("ERROR",$g_routine_name,"Cannot find file $temporary_compressed_filename");
            $o_decompress_status = 1;
        } elsif ($? == 0){
            $o_decompress_status = 0;
        } else {
        ...
        $o_uncompressed_filename = $o_full_output_filename;  # The file was not compressed at all, we set the output name to the input name.
        log_this("INFO",$g_routine_name,"ORIGINAL_FILE_IS_UNCOMPRESSED_ALREADY $o_uncompressed_filename");
    }

    if ($g_debug) { 
        print "$function_name: o_decompress_status [$o_decompress_status] o_uncompressed_filename [$o_uncompressed_filename]\n";
    }
...
sub apply_file_management_action {
    # Given the input file name, output directory, and an action, function will apply that action to the file.
    # The normal use for this function is to move, copy or delete a file to a different directory.
    #     FILE_MOVE -> 'mv'
    #     FILE_COPY -> 'cp'

    my $i_file_action      = shift;  # This is the system command to apply to the file.  The actual command is in the file_management_lookup_table hash table.
    my $i_input_filename   = shift;
    my $i_output_directory = shift;

    my $routine_name = "apply_file_management_action";
    my $debug_module = "apply_file_management_action:";
    my $debug_mode   = 0; 

    my $o_file_action_status = 1;  # A value of 1 is successful and 0 is failed.
    my $o_full_output_filename = "";

    # Define supported actions in this function.
    my %file_management_lookup_table = (
        'FILE_MOVE',   'mv ',
        'FILE_COPY',   'cp ',
        'FILE_DELETE', 'rm -f ',
    );

    die("Unrecognized action $i_file_action.  Program exiting") if (not exists $file_management_lookup_table{$i_file_action});

    my $call_system_command_str = "";

    # Depends on the value of i_file_action, we build the system command.  Note that depend on the command, we apply the correct parameters.

    if ($i_file_action eq "FILE_MOVE") { 
       $call_system_command_str = $file_management_lookup_table{$i_file_action} . "$i_input_filename $i_output_directory";
       # Build the output filename.
       my $filename_only  = basename($i_input_filename);
       $o_full_output_filename = "$i_output_directory/$filename_only";
    } elsif ($i_file_action eq "FILE_COPY")    {
       $call_system_command_str = $file_management_lookup_table{$i_file_action} . "$i_input_filename $i_output_directory";
       # Build the output filename.
       my $filename_only  = basename($i_input_filename);
       $o_full_output_filename = "$i_output_directory/$filename_only";
    } elsif ($i_file_action eq "FILE_DELETE")  { 
       $call_system_command_str = $file_management_lookup_table{$i_file_action} . "$i_input_filename";
    } else {
        die("File action [$i_file_action] is not supported yet in this program.");
    }

    log_this("INFO",$routine_name,"$i_file_action [$call_system_command_str]");
    ...
    #
    if ($? == -1) {
        log_this("ERROR",$routine_name,"system [$call_system_command_str] failed to execute: $?");
        $o_file_action_status = 0;
    } elsif ($? == 256){
        log_this("ERROR",$routine_name,"Cannot find file $i_input_filename");
        $o_file_action_status = 0;
    } elsif ($? == 0){
        # This is successful and what we desire.
        $o_file_action_status = 1;
#            log_this("INFO",$routine_name,"$move_or_copy_command $i_input_filename $i_output_directory");
    } else {
        log_this("ERROR",$routine_name,"system [$call_system_command_str] executed with: $?");
        $o_file_action_status = 0;
    }

    # Do one final check of the output file if it is not an empty string, i.e. the expected output file from the FILE_MOVE and FILE_COPY command.
    if ($o_full_output_filename ne "") {
        if (not (-f $o_full_output_filename)) {
            $o_file_action_status = 0;  # A value of 1 is successful and 0 is failed.
            log_this("ERROR",$routine_name,"Expected output file does not exist $o_full_output_filename");
        }
    }

    return($o_file_action_status,$o_full_output_filename);
}
...
#------------------------------------------------------------------------------------------------------------------------
# Begin of main subroutine generic_level2_uncompressor.
#
...
my $debug_module = "generic_level2_uncompressor:";
my $module_name  = "generic_level2_uncompressor.pl";
my $debug_mode   = 0;
...
if (index($0,$module_name) >= 0)
{
    ...
    if ($num_args < 3 ) {
    	print "usage: perl generic_level2_uncompressor.pl datasource search_directory                                   num_files_to_uncompress value_move_instead_of_copy \n";
    	print "usage: perl generic_level2_uncompressor.pl MODIS_A    /data/dev/scratch/IO/modis/MODIS_AQUA_L2_SST_OBPG  30 yes\n";
    	print "usage: perl generic_level2_uncompressor.pl MODIS_A    /data/dev/scratch/IO/modis/MODIS_AQUA_L2_SST_OBPG  1  no\n";
    	print "usage: perl generic_level2_uncompressor.pl MODIS_T    /data/dev/scratch/IO/modis/MODIS_TERRA_L2_SST_OBPG 30 yes\n";
    	print "usage: perl generic_level2_uncompressor.pl MODIS_T    /data/dev/scratch/IO/modis/MODIS_TERRA_L2_SST_OBPG 1  no\n";
    	print "usage: perl generic_level2_uncompressor.pl VIIRS      /data/dev/scratch/IO/modis/VIIRS_L2_SST_OBPG       1  no\n";
    	print "usage: perl generic_level2_uncompressor.pl VIIRS      /data/dev/scratch/IO/modis/VIIRS_L2_SST_OBPG       1  yes\n";
    	exit(1);
    }
    ...
    use FindBin;
    use lib "$FindBin::Bin/.";
    use generic_downloader_job_manager;
    $g_gdjm = new generic_downloader_job_manager();

    $i_modis_data_name_prefix = $g_gdjm->get_crawl_info($i_processing_type);

    if ($ENV{GHRSST_OBPG_USE_2019_NAMING_PATTERN} eq "true") {
        $i_modis_data_name_prefix = "SNPP_VIIRS";
    }

    my $uncompressor_driver_status = generic_level2_uncompressor_driver($i_datatype, 
                                                                        $i_datasource, 
                                                                        $i_processing_type, 
                                                                        $i_modis_search_directory, 
                                                                        $i_modis_data_name_prefix, 
...
# End of of main subroutine generic_level2_uncompressor.
